...
define(function(require, exports, module) {
	...
	var userInfo = {
		...
		init: async function() {
			...
			oThis.today = await clientApi.serverTime('-');
		...
		initSecond: async function() {
			var oThis = this;
			await oThis.loadAndRenderUserName();
			await oThis.loadAndRenderUserLeftDays();
			oThis.loadTabInfo();
		...
		// 获取用户名
		loadAndRenderUserName:async function() {
			try {
				...
				$('#username').html(await clientApi.thsPassport.get('account'));
		...
		// 获取剩余时间和日期
		loadAndRenderUserLeftDays: async function() {
			var userLeftDays	= await thsClientCache.getCache('userLeftDays');
			if (userLeftDays) {
		...
		// 更新new图标的状态
		updateNewInfo: async function() {
			var oThis = this;
			
			try{
				var read2 = await storage.getFileContent("read2.log");
			} catch (e) {
				storage.writeFileContent("read2.log", '0');
				read2 = '0';
			}
			try{
				var read3 = await storage.getFileContent("read3.log");
			} catch (e) {
				storage.writeFileContent("read3.log", '0');
				read3 = '0';
			}
			try{
				var read4 = await storage.getFileContent("read4.log");
			} catch (e) {
				storage.writeFileContent("read4.log", '0');
				read4 = '0';
			}
			try{
				var read5 = await storage.getFileContent("read5.log");
			} catch (e) {
				storage.writeFileContent("read5.log", '0');
				read5 = '0';
			}
			try{
				var read6 = await storage.getFileContent("read6.log");
			} catch (e) {
				storage.writeFileContent("read6.log", '0');
				read6 = '0';
			}
			try{
				var read7 = await storage.getFileContent("read7.log");
			} catch (e) {
				storage.writeFileContent("read7.log", '0');
				read7 = '0';
			}
			try{
				var read8 = await storage.getFileContent("read8.log");
			} catch (e) {
					...
					$.each(result.result, function(advk, advObj) {
						switch (parseInt(advObj.case_id)) {
							...
							case oThis.advObejct[oThis.version]['leveltwo'] :
								// if (!$('#tab-line-0 .message-tab-line-right-title').eq(0).html()) {
								...
								// if (!$('#tab-line-1 .message-tab-line-right-title').eq(0).html()) {
							...
							case oThis.advObejct[oThis.version]['jdal'] :
								// if (!$('#tab-line-6 .message-tab-line-right-title').eq(0).html()) {
								...
								// if (!$('#tab-line-7 .message-tab-line-right-title').eq(0).html()) {
								...
								// if (!$('#tab-line-8 .message-tab-line-right-title').eq(0).html()) {
							...
							case oThis.advObejct[oThis.version]['jrjq'] :
								// if (!$('#tab-line-3 .message-tab-line-right-title').eq(0).html()) {
							...
							case oThis.advObejct[oThis.version]['three'] :
								// 早盘必读 9:02
								// if (!$('#tab-line-4 .message-tab-line-right-title').eq(0).html()) {
								...
								// 每日一招 19：45
								// if (!$('#tab-line-5 .message-tab-line-right-title').eq(0).html()) {
								...
								// 今日收评
								// if (!$('#tab-line-2 .message-tab-line-right-title').eq(0).html()) {
